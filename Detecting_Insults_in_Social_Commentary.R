...
trainSet = read.csv("data/train.csv", header = TRUE, na.strings=c("NA",""))
dim(trainSet)
str(trainSet)
head(trainSet)
...
testSet = read.table("data/test_with_solutions.csv", sep = ",", header = TRUE)
dim(testSet)
str(testSet)
head(testSet)
summary(testSet)
...
comment_test = removeSparseTerms(comment_test,0.97)
dim(comment_test)
